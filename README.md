...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.4.0 | 2026-06-25 | 监控窗口 v3：三列布局（生成中/排队中/已完成），每个视频独立进度条，pendingFiles 队列追踪；后端 API 支持停止/开始/删除缩略图 |
| v1.3.0 | 2026-06-25 | 监控窗口 v2：停止/开始生成、一键删除缩略图、失败列表、最近完成列表；后端新增 stop/start/delete API |
...
项目提供了一个 Python GUI 工具，可实时查看和控制缩略图生成状态：
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/thumbnail-progress | 获取缩略图生成进度（含队列、失败列表） |
| POST | /api/thumbnail/stop | 停止生成 |
| POST | /api/thumbnail/start | 开始/恢复生成 |
| DELETE | /api/thumbnails | 删除所有缩略图缓存 |
